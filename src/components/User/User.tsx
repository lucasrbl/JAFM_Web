...
import { UserType } from '../../types/Manager';
...
const User = (props: UserType) => {
  ...
  const getAverageProgress = (students: UserType[]): number => {
    ...
    const totalProgress = students.reduce(
      (acc, student) => acc + Number(student.progresso),
      0
    );
  ...
  const countStudents = (
    students: UserType[]
  ): { count: number; average: number; arr: number[] } => {
  ...
  console.log(
    'Turmas do professor atual',
    turma + '\nOutras turmas: ',
    otherClasses
  );
  console.log(
    'Média do progresso de usuários',
    res.average + '\nMédia do progresso de usuários de outras turmas',
    otherRes.average
  );
  console.log(
    'Progressos de usuários',
    res.arr + '\nProgressos de usuários de outras turmas',
    otherRes.arr
  );
  ...
  return (
    <div className='flex flex-col items-center justify-center p-5'>
        ...
        <div>
          <img
            ...
            alt={
              props.profileImg
                ? 'Foto de perfil'
                : 'Usuário sem foto de perfil disponível'
            }
            className='rounded-full w-40 overflow-hidden'
          />
        </div>
        <div className='flex flex-col'>
          <p className='font-vietnam text-white text-xl'>{props.name}</p>
          <p className='font-vietnam text-white text-sm'>
            <span className='font-bold'>RA:</span> {props.ra}
          </p>
          <p className='font-vietnam text-white text-sm'>
            <span className='font-bold'>Turma:</span> {props.turma}
          </p>
          <p className='font-vietnam text-white text-sm'>
            <span className='font-bold'>CNPJ da Empresa:</span> {props.cnpj}
          </p>
        </div>
      </div>
      ...
      <div className='flex flex-col justify-center items-start w-9/12'>
        ...
        <div>
          <p className='font-vietnam text-white text-sm'>
            Usuário cadastrado em {props.dataInicio}, tem como telefone de
            contato o número {props.telefone}, frequenta também a unidade de{' '}
            {props.turma == 'TIN5' || 'TIN6'
              ? 'Campinas'
              : props.turma == 'TIN1' || 'TIN2'
              ? 'Sorocaba'
              : 'São Paulo'}
            , nascido(a) em {props.dataNascimento}. Seu progresso é avaliado
            atualmente em:
          </p>
        </div>
      </div>
      ...
      <div className='flex gap-x-12 justify-center items-center mt-8 pb-10'>
        ...
        <div className='flex flex-col items-center'>
          ...
          <p className='font-vietnam text-white font-bold text-4xl'>
            {props.progresso}%
          </p>
        </div>
      </div>
      ...
      <div className='flex flex-col items-center gap-3'>
        <p className='font-vietnam text-white text-sm'>
          ...
          {Number(props.progresso) == 50
            ? 'progresso razoável, já que possui metade da pontuação possível de se atingir'
            : Number(props.progresso) >= 80
            ? 'ótimo progresso, possuindo grande destaque entre os jovens da filial em questão, com um desenvolvimento fantástico'
            : Number(props.progresso) > 65 && Number(props.progresso) < 80
            ? ' bom progresso com pontuação suficiente para estar com seu desenvolvimento confortável'
            : Number(props.progresso) <= 45
            ? 'progresso ruim, converse com o jovem em questão para debater pontos de melhoria identificados'
            : 'Não há dados'}
        </p>
        <p className='font-vietnam text-white text-sm'>
          Clique abaixo para ter uma visualização gráfica dos dados do jovem
          aprendiz
        </p>
      </div>
      ...
      <div>
        <button
          className='flex items-center gap-1 w-55 font-vietnam px-3 py-1 rounded-2xl text-sm bg-white'
